...
function getStyledHtml(input){
  var s = getStyles() + wrapHtml(input);
  return s;
}

function wrapHtml(input){
  return '<body class="markdown-preview native-key-binding markdown-body">' + input + '</body>';
}
...
function makePdf(inputHtml, outputPath){
  pdf.create(inputHtml).toFile(outputPath, function(err, res) {
    if (err) {
      ...
      var msg = "Converted successfully. Output in " + outputPath;
