...
function processSrc(src){
  ...
  if(url.parse(src).protocol){
    //if the src already starts with "file://", "https://", etc., it's already in the right form
    return src
  }
  else if(path.resolve(src) !== src){
    //if path is not absolute, and has no protocol, it should be relative
    src = path.resolve(markdownPath, src);
    return src;
  }
}
