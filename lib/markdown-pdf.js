...
module.exports = {
  ...
  config: {
    ...
    "converter": {
      "title": "Converter",
      "description": "Converting library to use (default: modified-html-pdf)",
      "type": "string",
      "default": "html-pdf",
      "enum": ["html-pdf", "markdown-pdf"]
    }
  },
  ...
  convert: function() {
    try{
      ...
      if(!mdpreview) { // Use markdown-preview-plus if markdown-preview has been disabled
        mdpreview = atom.packages.getActivePackage('markdown-preview-plus');
      }
      let conv = atom.config.get('markdown-pdf.converter');
      if (conv === 'html-pdf') {
      ...
      } else if (conv === 'markdown-pdf') {
        let activeEditor = atom.workspace.getActiveTextEditor()
        let inPath = activeEditor.getFileName();
        outPath = getOutputPath();
        let mdString = activeEditor.getText();
        let options = {
          source: inPath,
          destination: outPath,
          pdf: {
        ...
        lh.allowUnsafeEval(() => {
          ...
          .then((pdfPath) => console.log('PDF Path:', pdfPath))
  ...
  pdf.create(inputHtml, conf).toFile(outputPath, function(err, res) {
    if (err) {
      atom.notifications.addError(
        'Error converting to image format. Check console for more information.',
        {
          buttons: [{
            className: 'md-pdf-err',
            onDidClick: () => require('remote').getCurrentWindow().openDevTools(),
            text: 'Open console',
          }]
        }
      )
