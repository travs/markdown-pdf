...
function getStyles(){
  //store the styles in an object
  var styles = Object();

  //get markdown-preview's styles
  styles.previewStyles = atom.packages.activePackages["markdown-preview"].stylesheets[0][1];

  if(atom.themes.getActiveThemes()[0].stylesheets[0] === undefined || atom.themes.getActiveThemes()[1].stylesheets[0] === undefined){
    //that big `if` statement catches errors where the stylesheet for your active theme isn't exposed
    getDefaults(styles);
  }
  else{
  ...
  //add user's styles last to override others
...
function getDefaults(styles){
  //resort to loading atom-dark-syntax and atom-dark-ui styles
  //if we can't find active stylesheets
  for (x of atom.themes.getLoadedThemes()){
    if (x.name === 'atom-dark-syntax'){
      styles.syntaxStyles = x;
      if(styles.uiStyles){
        //we can end search if both are defined
        break;
      }
    }
    else if (x.name === 'atom-dark-ui'){
      styles.uiStyles = x;
      if(styles.syntaxStyles){
        break;
      }
    }
  }
