...
var converter = {
  ...
  htmlFromPreview: function () {
    //get markdown HTML using Atom's Markdown-Preview package
...
function makePdf(inputHtml, outputPath){
  atom.notifications.addInfo('Converting markdown to PDF...', {icon: 'markdown', dismissable: true})
  ...
  pdf.create(inputHtml, conf).toFile(outputPath, function(err, res) {
    if (err) {
      ...
      atom.notifications.addSuccess('Converted successfully.', {detail: 'Output in ' + outputPath, icon: 'file-pdf', dismissable: true});
    }
  });
}
